...
sub _fix_make_cuda {
	...
	# replace each of the cflags with '-Xcompiler \"...\"' and dittor for -Xlinker
	# this is dirty
...
# BLIAKO: modification: add --compiler-bindir <gcc> to nvcc
    ...
    for my $ak (qw/nvcc nvlink cc cxx ld/){
    	my $AK = uc $ak;
    ...
    for my $ak (qw/ccflags nvccflags lddlflags nvldflags/){
    	my $AK = uc $ak;
    	if( ! exists($o->{ILSM}{MAKEFILE}{$AK}) || ! defined($o->{ILSM}{MAKEFILE}{$AK}) ){ $o->{ILSM}{MAKEFILE}{$AK} = '' }
    }
    ...
    if( version->parse($conf->{'nvcc-release'}) > version->parse('10.2') ){
...
#    }
    ...
    $o->{ILSM}{MAKEFILE}{CC} = $o->{ILSM}{MAKEFILE}{NVCC}
        . ' --compiler-bindir '
        . '"' . $host_compiler_cmd . '"'
    ;
...
B<WARNING:> first, see section L</INSTALLATION> for how to install this package.
...
C<Inline->use(CUDA => "DATA")> seems to have a problem
